...
def detect_calibration_points(gray_image, pattern_size):
    ...
    refined_corners = cv2.cornerSubPix(gray_image, corners, (11, 11), (-1, -1), stop_criteria)
...
def calibrate_camera(object_points, image_points, image_size):
    ...
    if camera_matrix is None or dist_coeffs is None:
        raise RuntimeError("相机标定失败：OpenCV 未能返回有效的矩阵。")
    ...
    if not np.all(np.isfinite(camera_matrix)) or not np.all(np.isfinite(dist_coeffs)):
        raise ValueError("相机标定失败：内参矩阵或畸变系数中包含非有限数值 (NaN 或 Inf)。")
    #     raise a clear error
    print(f"Calibration successful. RMS re-projection error: {ret:.4f} pixels")
